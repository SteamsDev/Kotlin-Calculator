    ...
    fun lexicalAnalysis(lexemeList: List<String>) {
        ...
        if (!(lexemeList.first().startsWith("NUM")) && lexemeList.first() != "LPAREN" && lexemeList.first() != "PI"
            ...
            throw ArithmeticException(
                "Invalid syntax: cannot have an operator or rparen at the beginning of the" +
                        " expression"
            )
        }
        ...
        for (i in 0 until lexemeList.size - 1) {
            when (lexemeList[i]) {
                ...
                "LPAREN" -> if ((!lexemeList[i + 1].startsWith("NUM") && lexemeList[i + 1] != "PI"
                    ...
                    throw ArithmeticException(
                        "Invalid syntax: lparen must be followed by a numeric  value or" +
                                "another lparen"
                    )
                } else {
                    lParenCount++
                ...
                "RPAREN" -> if (lexemeList[i + 1].startsWith("NUM") || lexemeList[i + 1] == "PI"
                    ...
                    throw ArithmeticException(
                        "Invalid syntax: rparen must be followed by an operator or another " +
                                "rparen"
                    )
                } else {
                    rParenCount++
                ...
                "PI", "E" ->
                    if (lexemeList[i + 1].startsWith("NUM") || lexemeList[i + 1] == "PI"
                        ...
                        throw ArithmeticException(
                            "Invalid syntax: numeric elements must be followed " +
